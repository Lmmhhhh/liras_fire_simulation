  ...
  export default function FireSimulation() {
    ...
    const handleDemUpload = useCallback(async e=>{
      ...
      const text=await f.text(),
            {data,nrows,cellSize:cs}=parseAscFile(text);
    ...
    const handleFuelUpload = useCallback(async e=>{
      ...
      const text=await f.text(),
            {data}=parseAscFile(text,terrainData.size);
    ...
    const initializeGrid = useCallback(()=>{
      ...
      const grid=Array.from({length:size},()=>Array.from({length:size},()=>({
        state:FIRE_STATES.UNBURNED, intensity:0
      })));
      setFireGrid(grid);
    },[size]);
    ...
    const spreadFire = useCallback((grid,dt)=>{
      ...
      const w = weatherData[Math.floor(time)]||{};
      ...
      for(let i=0;i<size;i++){
        for(let j=0;j<size;j++){
          ...
          const dist=ros*dt*speed, steps=Math.ceil(dist/cellSize);
          for(let di=-steps;di<=steps;di++)for(let dj=-steps;dj<=steps;dj++){
      ...
      terrainData,weatherData,fireGrid,computeSlopeFn,
      fuelModelData,fuelMoistureData,size,cellSize,time,speed
    ]);
    ...
    useEffect(()=>{
      ...
      const iid=setInterval(()=>{
        setTime(t=>t+1);
        setFireGrid(g=>g?spreadFire(g,1):g);
      },1000);
      return ()=>clearInterval(iid);
    },[isRunning,spreadFire]);
  
    // 로딩 화면
    if(!terrainData||!fuelModelData||!fuelMoistureData||!canopyCoverData||!weatherData){
      return (
        <div className="p-6">
          <DataUploader
    ...
    const currentWeather=weatherData[Math.floor(time)]||{};
